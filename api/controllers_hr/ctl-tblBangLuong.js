...
async function take7thDataToWork(db, year, month, dateRequest = 10) {
    ...
    await mtblConfigWorkday(db).findAll({
        where: {
            Date: {
                [Op.between]: [dateMonthFirst, dateWhere]
            },
        },
        order: [
            ['Date', 'DESC']
                ...
                await tblDMNhanvien.findAll({
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if(data[i].remainingSpellsUpdateDate 
                            ...
                            remainingPreviousYear = await calculateRemainingPreviousYear(db, data[i].ID, year + '-' + await convertNumber(month));
                        }
...
async function getDataTimeKeeping(dateRes, departmentID) {
    ...
    await database.connectDatabase().then(async db => {
        // await axios.get(`http://192.168.23.9:1333/dulieuchamcong/`).then(data => {
        //     if (data.length > 0)
...
async function getDetailSyntheticTimkeeping(db, departmentID, dateStart, dateEnd) {
    ...
    await mtblTimeAttendanceSummary(db).findAll({
        ...
        for (let i = 0; i < data.length; i++) {
            let remainingPreviousYear = data[i].RemainingPreviousYear
            let remaining = data[i].Remaining
            // Nếu admin cấu hình lại số phép tồn của nhân viên vào tháng search thì sẽ thay bằng tháng đó
            await mtblDMNhanvien(db).findOne({
                where: { 
                    ID: data[i].StaffID,
                }
            }).then(staff => {
                if(staff.remainingSpellsUpdateDate 
                    && Number(moment(staff.remainingSpellsUpdateDate).format('DD')) === monthStart) {
                        remainingPreviousYear = _.get(staff, 'remainingSpells', 0)
                        remaining = remainingPreviousYear + Number(data[i].Overtime) - Number(data[i].LateDay) - Number(data[i].NumberHoliday);
                    } 
            })
            array.push({
...
module.exports = {
    ...
    deleteAllTimekeeping: async(req, res) => {
        let body = req.body;
        await axios.get(`http://192.168.23.9:1333/dulieuchamcong/deleteall`).then(data => {
            if (data == 'done') {
                var result = {
                    status: Constant.STATUS.SUCCESS,
                    message: Constant.MESSAGE.ACTION_SUCCESS,
    ...
    deleteAndCreateDataTimeKeeping: async(req, res) => {
        database.connectDatabase().then(async db => {
            if (db) {
                let arrayData = []
                await axios.get(`http://192.168.23.9:1333/dulieuchamcong/index`).then(data => {
